...
**Guided Encouragement** is a real feature from [Bloom](https://muffin.co.jp), muffin Inc.'s LLM service — Next.js/TypeScript API with Server-Sent Events streaming from the Claude API, zod-validated typed contract consumable by the client, server-side key management and rate limiting. After clearing a stage, the player labels how they feel — a light *affect labeling* moment — and receives a short, warm encouragement matched to that feeling from Mamorin, Bloom's sleep-loving mascot, generated by Claude. This repo is a portfolio prototype of that feature: the web UI simulates Bloom's post-stage screen, while the API route is built as the actual production-shaped service the Unity client would call — same contract, streaming, and the API key never leaves the server.

## LLM / AI-powered features:
- Streaming Claude API service + safety design: 
	https://github.com/muffin-jp/llm-guided-encouragement/pull/1
- Automated LLM eval harness in GitHub Actions CI: 
	https://github.com/muffin-jp/llm-guided-encouragement/pull/2
...
The committed `evals/results/latest.md` is refreshed by every real run (CI or local). The snapshot below is from the latest online test
#### Guided Encouragement — Eval Results

- Generated: 2026-07-28T05:25:32.878Z
- Cases: 50
...
| Mean empathy | 4.53 | 4.00 | 40 | ✅ |
| Mean tone | 4.58 | 4.00 | 40 | ✅ |
